...
protected:
	FramePool(const uint32_t &max_frame_num = DEFAULT_MAX_FRAME_NUM,
	...
	:	MAX_FRAME_NUM(max_frame_num > 0 ? max_frame_num : DEFAULT_MAX_FRAME_NUM),
		...
		init_pool(init_frame_num < MAX_FRAME_NUM ? init_frame_num : MAX_FRAME_NUM, default_frame_sz);
	...
	virtual T *create_frame(const size_t &data_bytes) {
		...
		if (LIKELY(init_num > 0)) {
			pool_mutex.lock();
			{
				...
				for (uint32_t i = 0; i < init_num; i++) {
					mFramePool.push_back(create_frame(default_frame_sz));
					total_frame_num++;
				}
				cleared = false;
				pool_sync.broadcast();
