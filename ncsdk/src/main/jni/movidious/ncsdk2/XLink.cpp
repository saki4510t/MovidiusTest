...
int handleIncomingEvent(xLinkEvent_t *event) {
	...
	case USB_WRITE_REQ:
		...
		if (addNewPacketToStream(stream, buffer, event->header.size)) {
			...
			deallocateData(buffer, ALIGN_UP(event->header.size, __CACHE_LINE_SIZE),
			  __CACHE_LINE_SIZE);
...
int dispatcherLocalEventGetResponse(xLinkEvent_t *event, xLinkEvent_t *response) {
	...
	case USB_WRITE_REQ: {
		...
		if (!isStreamSpaceEnoughFor(stream, event->header.size)) {
			...
			mvLog(MVLOG_DEBUG,
			  "Got local write of %ld , remote fill level %ld out of %ld %ld\n",
	...
	case PCIE_READ_REQ: {
		...
		printf("Xlink Read with data size %d and data %p \n",
		  (int) ((streamPacketDesc_t *) event->data)->length,
		  (streamPacketDesc_t *) event->data);
