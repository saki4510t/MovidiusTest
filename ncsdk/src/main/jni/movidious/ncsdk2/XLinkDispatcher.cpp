...
char *TypeToStr(int type) {
	...
	case USB_READ_REL_REQ:
		return "USB_READ_REL_REQ";
	case USB_CREATE_STREAM_REQ:
		return "USB_CREATE_STREAM_REQ";
	case USB_CLOSE_STREAM_REQ:
		return "USB_CLOSE_STREAM_REQ";
	case USB_PING_REQ:
		return "USB_PING_REQ";
	case USB_RESET_REQ:
		return "USB_RESET_REQ";
	case USB_REQUEST_LAST:
		return "USB_REQUEST_LAST";
	case USB_WRITE_RESP:
		return "USB_WRITE_RESP";
	case USB_READ_RESP:
		return "USB_READ_RESP";
	case USB_READ_REL_RESP:
		return "USB_READ_REL_RESP";
	case USB_CREATE_STREAM_RESP:
		return "USB_CREATE_STREAM_RESP";
	case USB_CLOSE_STREAM_RESP:
		return "USB_CLOSE_STREAM_RESP";
	case USB_PING_RESP:
		return "USB_PING_RESP";
	case USB_RESET_RESP:
		return "USB_RESET_RESP";
	case USB_RESP_LAST:
		return "USB_RESP_LAST";
	default:
		break;
	}
	return "";
...
static inline xLinkEventPriv_t *
getNextElementWithState(xLinkEventPriv_t *base, xLinkEventPriv_t *end,
  xLinkEventPriv_t *start, xLinkEventState_t state) {
...
static xLinkEventPriv_t *searchForReadyEvent(xLinkSchedulerState_t *curr) {
	...
	ev = getNextElementWithState(curr->lQueue.base, curr->lQueue.end, curr->lQueue.base,
	  EVENT_READY);
	if (ev) {
...
static void dispatcherReset(xLinkSchedulerState_t *curr) {
	...
	event = getNextElementWithState(curr->lQueue.base, curr->lQueue.end, curr->lQueue.base,
	  EVENT_PENDING);
	while (event != NULL) {
		markEventServed(event);
		event = getNextElementWithState(curr->lQueue.base, curr->lQueue.end, curr->lQueue.base,
