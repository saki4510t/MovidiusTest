...
static const char *getProductName(const char *name)
{
...
{
    return (ncOptionClass_t) ((option - base) / OPTION_CLASS_SIZE);
}
...
static void resetAll()
{
    ...
    while (1) {
        size_t name_size = NC_MAX_NAME_SIZE;
        rc = XLinkGetDeviceName(index, name, name_size);
        ...
        if (strlen(getProductName(name)) == 1) {    //name doesn't have product number
            ...
            XLinkHandler_t *handler = (XLinkHandler_t *)calloc(1, sizeof(XLinkHandler_t));
    ...
    if (stalled_count) {
        ...
        while ((after_reset_count < bootrom_count + stalled_count) &&
            ...
            size_t name_size = NC_MAX_NAME_SIZE;
            while (1) {
                XLinkError_t rc = XLinkGetDeviceName(index, name, name_size);
                if (rc != X_LINK_SUCCESS)
                    break;  //no more devices found
...
ncStatus_t ncDeviceCreate(int index, struct ncDeviceHandle_t **deviceHandle)
{
    ...
    if (rc == X_LINK_SUCCESS) {
        struct ncDeviceHandle_t *dH = (struct ncDeviceHandle_t *)calloc(1, sizeof(*dH));
        struct _devicePrivate_t *d = (struct _devicePrivate_t *)calloc(1, sizeof(*d));
        dH->private_data = d;
...
{
    if (!d->thermal_stats) {
        d->thermal_stats = (float *)calloc(NC_THERMAL_BUFFER_SIZE + sizeof(float), 1);   //extra space for throttling level
        if (!d->thermal_stats)
...
ncStatus_t ncDeviceOpen(struct ncDeviceHandle_t * deviceHandle)
{
    ...
    while (timeInSeconds() < waittm && rc == 0) {
        ...
        if (rc != X_LINK_SUCCESS) {
            ...
            while (1) {
                name2[0] = '\0';
                rc = XLinkGetDeviceName(count, name2, name_size);
                if (rc != X_LINK_SUCCESS)
                    break;
                         ...
                         struct ncGraphHandle_t ** graphHandle)
{
    ...
    struct ncGraphHandle_t *gH = (struct ncGraphHandle_t *)calloc(1, sizeof(*gH));
    struct _graphPrivate_t *g = (struct _graphPrivate_t *)calloc(1, sizeof(*g));
    gH->private_data = g;
                           ...
                           unsigned int graphBufferLength)
{
    ...
    if (XLinkWriteData(streamId, (const uint8_t *)graphBuffer, graphBufferLength) != 0) {
                            ...
                            unsigned int dataLength)
{
    ...
    case NC_OPTION_CLASS1:
        rc = setGraphOptionClass1(g, (ncGraphOption_t)option, data, dataLength);
        break;
#ifndef EXCLUDE_HIGHCLASS
                            ...
                            int option, void *data, unsigned int *dataLength)
{
    ...
    ncOptionClass_t clazz = getOptionClass(option, GRAPH_CLASS0_BASE);
    if (g->dev != NULL && clazz > g->dev->dev_attr.max_graph_opt_class) {
        mvLog(MVLOG_ERROR, "This device FW does not support NC_OPTION_CLASS%d",
		  clazz);
        return NC_UNAUTHORIZED;
    }
    ...
    switch (clazz) {
    case NC_OPTION_CLASS0:
        rc = getGraphOptionClass0(g, (ncGraphOption_t)option, data, dataLength);
        break;
    case NC_OPTION_CLASS1:
        rc = getGraphOptionClass1(g, (ncGraphOption_t)option, data, dataLength);
        break;
#ifndef EXCLUDE_HIGHCLASS
    case NC_OPTION_CLASS2:
...
ncStatus_t ncGlobalGetOption(int option, void *data, unsigned int *dataLength)
{
    ...
    case NC_RO_API_VERSION:{
            ...
            if (api_version[0] == 0 && strlen(NCSDK_VERSION_NAME) != 0) {
                int res =
                    sscanf(NCSDK_VERSION_NAME, "%2d.%2d.%2d.%2d", &api_version[0],
                             ...
                             int option, void *data, unsigned int *dataLength)
{
    ...
    switch (opClass) {
    case NC_OPTION_CLASS0:
        rc = getDeviceOptionClass0(d, (ncDeviceOption_t)option, data, dataLength);
        break;
        //no class1 - no write options for device
#ifndef EXCLUDE_HIGHCLASS
    case NC_OPTION_CLASS2:
                           ...
                           void *userParam)
{
    ...
    if (handle->host_tensor_desc.dataType == NC_FIFO_FP32 || layout != host_layout) {
        unsigned char *inputTensorConverted = (unsigned char *)malloc(inputDesc->totalSize);
        ...
        rc = XLinkWriteData(handle->streamId, (const uint8_t *)inputTensor, *inputTensorLength);
    }
    if (rc != 0)
        return NC_ERROR;
                          ...
                          unsigned int *outputDataLen, void **userParam)
{
    ...
    if (!XLinkReadData(handle->streamId, &packet)) {
        ...
        if (handle->host_tensor_desc.dataType == NC_FIFO_FP32 ||
            ...
            convertDataTypeAndLayout((unsigned char*) packet->data, (unsigned char*)outputData,
                           ...
                           const void *data, unsigned int dataLength)
{
    ...
    case NC_RW_FIFO_TYPE:{
		...
		ncFifoType_t tempType = *(ncFifoType_t *) data;
    ...
    case NC_RW_FIFO_DATA_TYPE:{
			...
			ncFifoDataType_t tempDType = (ncFifoDataType_t)(*(int *) data);
                                 ...
                                 unsigned int outFifoCount)
{
    ...
    rc = (ncStatus_t)pushUserParam(fo, user_param, 0);
    if (rc != NC_OK) {
