...
UsbDataLink::UsbDataLink(jobject obj)
:	movidius_obj(obj),
	device(NULL),
	active(false)
{
...
void UsbDataLink::release(JNIEnv *env) {
	ENTER();
...
int UsbDataLink::connect(const int &fd) {
	ENTER();
	...
	if (device && device->is_active()) {
		...
		LOGE("could not find UsbDataLink:err=%d", result);
		SAFE_DELETE(device);
	}
...
const bool UsbDataLink::is_active() const {
	return active && device && device->is_active();
};

int UsbDataLink::reset() {
	ENTER();
...
int UsbDataLink::get_status(myriadStatus_t &myriad_state) {
	ENTER();
...
/*private*/
int UsbDataLink::write(const void *data, const size_t &size) {
	ENTER();
...
/*private*/
int UsbDataLink::read(void *data, const size_t &size) {
	ENTER();
